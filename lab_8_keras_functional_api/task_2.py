...
from keras.layers import BatchNormalization, Conv2D, Dense, Add, AveragePooling2D, Flatten
...
def add_res_net_module(input_tensor, last: bool):
    path = Conv2D(filter_cnt, (3, 3), padding='same', activation='selu')(input_tensor)
    path = BatchNormalization()(path)
    if not last:
        path = Dense(32, activation='relu')(path)
...
for i in range(res_net_cnt):
    output_tensor = add_res_net_module(output_tensor, i == (res_net_cnt - 1))
...
plot_model(ANN, to_file='task_2_model.png', show_shapes=True)
